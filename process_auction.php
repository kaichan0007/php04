...
<!-- Main[Start] -->
<form method="POST" action="process_auction2.php">
  <div class="jumbotron">
   <fieldset>
   <legend>オークション処理</legend>
     <label>車両名:<?=$row["vehicle_name"]?></label><br>
     <label>VIN number: <?=$row["VIN_number"]?></label><br>
     <label>登録日: <?=$row["purchase_date"]?></label><br>
     <label>ステータス更新（落札有=2/落札無=0）: 
      
     <select name='auction_flag'>
      <option value='2'>落札有</option>
      <option value='0'>落札無</option>
     </select>
