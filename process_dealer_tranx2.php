<?php
//1. POSTデータ取得
$transaction_dealer = $_POST["transaction_dealer"];
$id    = $_POST["id"];   //idを取得
...
//３．データ登録SQL作成
$sql = "update used_stock_table set owner_id=:owner_id, purchase_date=:purchase_date, transaction_status=0, transaction_dealer='' where id=:id";
$stmt = $pdo->prepare($sql);
$stmt->bindValue(':owner_id',$transaction_dealer, PDO::PARAM_STR);  //Integer（数値の場合 PDO::PARAM_INT)
...
if($status==false){
    ...
    $test_alert = "<script type='text/javascript'>alert('DLR間取引処理が完了しました！');</script>";
    echo $test_alert;
